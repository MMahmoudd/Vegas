        ...
        <div class="tatal-details d-flex justify-content-between">
          ...
          <div class="total">
            <p class="font-weight-bold mr-3">
              <span class="header-section">{{ $t("cart.subtotal") }}:</span>
              {{ total }} L.E
            </p>
          </div>
        </div>
      ...
      <div class="d-flex">
        ...
        <div class="content">
          ...
          <div v-if="product.selectedAddons">
            <div
              class="row border"
              v-for="addon in product.selectedAddons"
              :key="addon.id"
            >
              <p class="col-md-6">
                {{ $t("cart.addOnName") }}:
                <span class="price">{{ addon.name_translate }}</span>
              </p>
              <p class="col-md-6">
                {{ $t("cart.addOnPrice") }}:
  ...
  computed: {
    ...
    total() {
      return this.$store.state.products.reduce(
        (p, item) => p + +item.totalPriceWithAddons,
        0
      );
    },
    // totalWithAddons() {
